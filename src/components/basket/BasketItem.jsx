...
export const BasketItem = ({ title, price, amount, id }) => {
  ...
  const incrementFoodHandler = async () => {
    try {
      ...
      return dispatch(
        snackbarActions.doSuccess({ message: `${title} Успешно добавлено` })
      );
    } catch (error) {
      return dispatch(
        snackbarActions.doError({ message: `${title} Что то пошло не так` })
      );
    }
  };
  ...
  const decrementFoodHandler = async () => {
    try {
      ...
      dispatch(
        snackbarActions.doSuccess({ message: `${title} Успешно удалено` })
      );
    } catch (error) {
      dispatch(
        snackbarActions.doError({ message: `${title} Что то пошло не так` })
